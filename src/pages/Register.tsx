...
const Register: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      if (!data.user?.email_confirmed_at) {
        setErrorToast("Check your email to confirm registration.");
        setTimeout(() => setErrorToast(null), 1000);
        navigate("/login");
      } else {
        navigate("/landing");
      }
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen font-sans">
        ...
        <div className="px-2 sm:px-28">
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center text-sm gap-2 sm:gap-0">
              ...
              <a
                href="#"
                className="text-blue-800 underline font-bold hover:underline"
              >
                T & C
              </a>
            </div>
      ...
      {/* Right: Image Section */}
      <div
        className="hidden md:flex w-2/5 bg-cover bg-center items-center justify-center"
